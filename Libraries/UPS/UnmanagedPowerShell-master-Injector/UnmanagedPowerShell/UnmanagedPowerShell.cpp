	...
	while (hPipe != INVALID_HANDLE_VALUE)
	{
		if (ConnectNamedPipe(hPipe, NULL) != FALSE)   // wait for someone to connect to the pipe
		{
			...
			if (read)
			{
				...
				{
					InvokeMethod(spType, L"InvokePS", str);
				}
			}
		}
...
void InvokeMethod(_TypePtr spType, wchar_t* method, wchar_t* command)
{
	...
	else
	{
		...
		char *str = (char*)malloc(SysStringByteLen(vtPSInvokeReturnVal.bstrVal));
		wcstombs(str, vtPSInvokeReturnVal.bstrVal, SysStringByteLen(vtPSInvokeReturnVal.bstrVal));

		if (hPipe != INVALID_HANDLE_VALUE)
		{
			WriteFile(hPipe,
				str,
				SysStringByteLen(vtPSInvokeReturnVal.bstrVal),   // = length of string + terminating '\0' !!!
				&dwWrittenNew,
				NULL);
		}
	}
